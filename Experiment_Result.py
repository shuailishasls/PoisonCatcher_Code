...
from Experiment_5_Identify_Attacked_Data import Identify_Attacked_Data, Identify_Attacked_Data_normal
...
def experiment_5_draw_picture(result_df, style_map, file_name):
	"""
	根据攻击模式绘制 F2 分数的子图
	:param result_df: 包含特征名、时间、attacked_mode、attack_ratio F2score 的 DataFrame
	"""
	# 按攻击模式分组
	...
	for i, (attack_mode, group) in enumerate(grouped):
		ax = axes[i]
		for attacked_feature, ratio_f2 in group.groupby('attacked_feature'):
			ratio_f2 = ratio_f2.groupby('attack_ratio')['F2 score'].mean()
			line, = ax.plot(ratio_f2.index, ratio_f2.values, label=attacked_feature,
			...
			if attacked_feature not in legend_labels:
				legend_handles.append(Line2D([0], [0], color=style_map[attacked_feature]['color'],
				                             marker=style_map[attacked_feature]['marker'], linestyle='-'))
...
if __name__ == "__main__":
	...
	exp = input('实验几？')
	times = 6 if exp == '5' else 32  # 实验5原本是3
	
	epsilon, confidence_level, threshold = 0.1, 0.9, 0.5
	...
	result_df_5, result_df_5_1 = pd.DataFrame(), pd.DataFrame()
	
	# country_attacked_ratio = (np.arange(0, 52, 3).astype(float) / 100).tolist()  # 被攻击的国家比例
	country_attacked_ratio = [0.05]  # 被攻击的国家比例
	...
	# ---------------------------开始实验---------------------------
	for _ in tqdm(range(2)):  # 随机多次以去除随机性
		_ = [f.unlink() for f in Path('./File/Attacked_Dataset').iterdir() if f.is_file()]  # 清空文件夹
		_ = [f.unlink() for f in Path('./File/Attacked_Dataset_User').iterdir() if f.is_file()]  # 清空文件夹
		_ = [f.unlink() for f in Path('./File/Attacked_Dataset_User_Time/').iterdir() if f.is_file()]  # 清空文件夹
		
		# 攻击数据生成
		generate_attacked_df(epsilon, discrete_attrs, attack_countries, discrete_attrs + continue_attrs, attacked_modes,
		                     country_attacked_ratio, current_ds, dfs_LDP_dict, dfs_dict, alpha_df, true_attack_ratio,
		                     processed_data, processed_data_non_LDP)
		
		if times == 32:
			result_df_temp1, result_df_bias_1 = Experiment_1_SQR_Bias_In_Attack_Ratio_Change(discrete_attrs,
			                                                                                 origin_sqr.copy(),
			                                                                                 alpha_df)
			result_df_1 = pd.concat([result_df_1, result_df_temp1], ignore_index=True)
			
			result_df_temp2, result_df_bias_2 = Experiment_2_SQR_Corr_Analysis(continue_attrs, discrete_attrs,
			                                                                   origin_sqr.copy(), confidence_level)
			...
			result_df_temp5 = Identify_Attacked_Data('./File/Attacked_Dataset_User', history_ds)
			result_df_5 = pd.concat([result_df_5, result_df_temp5], ignore_index=True)
			
			result_df_temp5_1 = Identify_Attacked_Data_normal('./File/Attacked_Dataset_User_Time/')
			result_df_5_1 = pd.concat([result_df_5_1, result_df_temp5_1], ignore_index=True)
	...
	if times == 32:
		...
		result_df_5.to_csv(f'./File/Experiment_6/epsilon_vs_f2_score/Experiment_6/' + str(epsilon) + '.csv', index=False)
		result_df_5_1.to_csv(f'./File/Experiment_6/epsilon_vs_f2_score/Experiment_6_1/' + str(epsilon) + '.csv', index=False)
		# result_df_5.to_csv(f'./File/Experiment_5_Identify_Attacked_Data' + str(epsilon) + '.csv', index=False)
		# result_df_5_1.to_csv(f'./File/Experiment_5_1_Identify_Attacked_Data' + str(epsilon) + '.csv', index=False)
	...
	# experiment_5_draw_picture(result_df_5, style_map, './File/PDF/Experiment_5_Identify_Attacked_Data.pdf')  # 实验3绘图
	# experiment_5_draw_picture(result_df_5_1, style_map, './File/PDF/Experiment_5_1_Identify_Attacked_Data.pdf')  # 实验3绘图
